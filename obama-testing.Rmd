...
- Discuss the 95% Confidence Interval of the survey. You can calculate this very simply sqrt(p(1-p)/n), or by calculating standard deviation of the posterior. Want to use Confidence Intervals/SE more. 
...
# Weighted takes into qccount the number of people predicted to support Obama, as well as their relative proportions in the context of the entire posterior. Add 1 to prevent multiplying by 0. 
  
# Also, adding 1 to total most likely makes all the math incorrect by some factor. Not sure how wrong everything would be, but I'm guessing it wouldn't be terribly off. 

  mutate(all_weighted = all_summary*(total+1), young_weighted = young_summary*(total+1)) %>%
  
# Find the maximum weighted value for each possible number of obama supporters in each room size. 
  
  rowwise() %>%
  mutate(max = max(c_across(c("all_weighted", "young_weighted")))) %>%
  ungroup() %>%
  
# Find the number of people who are different in the two rooms. 
  
  mutate(diff = abs(all_weighted - young_weighted)) %>%
...
# Number of people who are Obamba supporters are independent, so multiply all the probabliities to get the answer. 
...
visualization <- inner_join(all, young, by = c("num_people", "total")) %>%
  ...
  select(-c(all_weighted, young_weighted))
